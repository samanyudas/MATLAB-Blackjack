...
% Function is the core of the blackjack game which implements and binds all of the code in the folder together.
    ...
    while true % Outer 'while loop' to re-run game as per exit request option chosen by user to replay the game
        ...
        while userBalance > 0
            ...
            fprintf('Tip: Run strategyGuide() in another window with your hand total and dealer''s upcard for advice!\n');
            ...
            fileID = fopen('game_log.txt', 'a'); % Opening file in append mode
            if fileID == -1
                fprintf('Error opening file for writing.\n');
            else
                fprintf(fileID, 'Game in round %d on %s with final balance: $%0.0f\n', roundCount, datetime(), userBalance); % Saving to File
                fclose(fileID); % Close File
                fprintf('Round balance logged to game_log.txt\n');
            end


            % Dialog to show the current round number
            uialert(uifigure, sprintf('Round %d completed', roundCount), 'Round Update');
        end
        ...
        if strcmp(playAgain, 'no')
            % Logging final balance to text file
            fileID = fopen('game_log.txt', 'a'); % Opening file in append mode
            if fileID == -1
                fprintf('Error opening file for writing.\n');
            else
                fprintf(fileID, 'Game ended on %s with final balance: $%0.0f\n', datetime(), userBalance); % Saving to File
                fclose(fileID); % Close File
                fprintf('Final balance logged to game_log.txt\n');
            end
            fprintf("\nGame Over!");
